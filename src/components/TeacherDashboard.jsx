...
import { CURRICULUM_DATA, getGradeData } from '../data/curriculumData';
import { 
  getStudents, 
  addStudent, 
  updateStudent, 
...
export function TeacherDashboard({ onLaunchQuest, onPrintQuest, activeTab, setActiveTab, onBackToPublic, onSignOut, currentTeacher }) {
  ...
  // Generator Grade data (dynamic loading)
  const [fullGenGradeData, setFullGenGradeData] = useState(null);

  useEffect(() => {
    let active = true;
    getGradeData(generatorGrade).then((data) => {
      if (active) setFullGenGradeData(data);
    });
    return () => { active = false; };
  }, [generatorGrade]);

  const genGradeData = useMemo(() => {
    if (fullGenGradeData && fullGenGradeData.grade === Number(generatorGrade)) {
      return fullGenGradeData;
    }
    return CURRICULUM_DATA.find((g) => g.grade === Number(generatorGrade)) || CURRICULUM_DATA[0];
  }, [generatorGrade, fullGenGradeData]);
  ...
  return (
    <div className="handbook-container">
      ...
      {/* Extracted Header and Tabs Component */}
      <TeacherHeader
